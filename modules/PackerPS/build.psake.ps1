...
Properties {
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $Build = @{
        IsRunningOnTeamCity = Test-Path Env:TEAMCITY_VERSION
    }
}
...
Task Test -depends Build -requiredVariables TestRootDir, ModuleName, CodeCoverageEnabled, CodeCoverageFiles {
    if ($build.IsRunningOnTeamCity) {
        Write-Output "##teamcity[progressStart '${taskName}']"
    }
    ...
    try {
        ...
        if ($CodeCoverageEnabled) {
            ...
            if ($build.IsRunningOnTeamCity) {
                Write-Output "##teamcity[buildStatisticValue key='CodeCoverageTotal' value='$($testResult.CodeCoverage.NumberOfCommandsAnalyzed)']"
                Write-Output "##teamcity[buildStatisticValue key='CodeCoverageCovered' value='$($testResult.CodeCoverage.NumberOfCommandsExecuted)']"
                Write-Output "##teamcity[buildStatisticValue key='CodeCoverageMissed' value='$($testResult.CodeCoverage.NumberOfCommandsMissed)']"
                Write-Output "##teamcity[buildStatisticValue key='CodeCoveragePercentage' value='${testCoverage}']"
            }
    ...
    if ($build.IsRunningOnTeamCity) {
        Write-Output "##teamcity[progressFinish '${taskName}']"
    }
}
